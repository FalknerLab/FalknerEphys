...
def process_single_location(location_data, path_lookup, param):
    ...
    try:
        ...
        conn, cursor = create_robust_db_connection(r"C:\Users\suyash\UCL\DeepUnitMatch\DeepMatch\matchtables.db")
        ...
        if retry_db_operation(_check_if_processed):
        ...
        wave_paths, unit_label_paths, channel_pos = paths_from_KS(KS_dirs)
        param = get_probe_geometry(channel_pos[0], param)

        waveform, session_id, session_switch, within_session, good_units, param = load_good_waveforms(wave_paths, unit_label_paths, param, good_units_only = False)
        waveform, session_switch, within_session, session_id, good_units, param = filter_good_units_and_merge(mt_path, mouse, KS_dirs, waveform, session_switch, param)
        ...
        extracted_wave_properties = extract_parameters(waveform, channel_pos, clus_info, param)

        # STEP 2, 3, 4
        # Extract metric scores
        total_score, candidate_pairs, scores_to_include, predictors  = extract_metric_scores(extracted_wave_properties, session_switch, within_session, param, niter  = 2)
        ...
        parameter_kernels = get_parameter_kernels(scores_to_include, labels, cond, param, add_one = 1)
        probability = apply_naive_bayes(parameter_kernels, priors, predictors, param, cond)

        add_col_to_sql(conn, cursor, mouse, probe, loc, "MatchProbNew", probability[:, 1], overwrite=True)
        conn.close()
...
if __name__ == "__main__":
    ...
    paths = pd.DataFrame(read_datapaths(os.listdir(root), r'/server/data/base'))
    valid_locs = get_valid_locations(root)
