...
from behavior import behavioral_raster


def run_knn_class(u_data, class_data, max_n=10, rand_state=42, test_ratio=0.2):
    X_train, X_test, y_train, y_test = train_test_split(u_data, class_data, test_size=test_ratio, random_state=rand_state)
    f1s = []
...
def run_reg_decoder(x_data, target_vars, model='glm', k=0):
    train_input, test_input, train_output, test_output = train_test_split(x_data, target_vars, test_size=0.2, random_state=42)
    model_obj = None
    if model == 'svm':
        model_obj = svm.SVR()
    elif model == 'glm':
